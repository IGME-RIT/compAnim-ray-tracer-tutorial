...
GLuint buffer;
int bufferSize = sizeof(triangle) * 14 + sizeof(light) * 1;
...
GLuint time_loc;	// Specifices where "time" is, in the GLSL shader
...
void init()
{
	...
	// Time, for animation
	time_loc = glGetUniformLocation(transform_program, "time");
